...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/popup-blocked') setError(t('auth.googleFail') + ' (popup blocked)');
      else if (err.code === 'auth/popup-closed-by-user') setError('');
      else setError(t('auth.googleFail'));
    } finally { setLoading(false); }
  };
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-50">
      <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md">
        ...
        <p className="text-center text-sm text-gray-500 mt-4">
          {t('auth.noAccount')} <a href="/register" className="text-blue-600 hover:underline ml-1">{t('auth.register')}</a>
        </p>
        <p className="text-center text-sm text-gray-500 mt-4">
          <a href="/" className="text-blue-600 hover:underline">{t('auth.backHome')}</a>
        </p>
      </div>
    </div>
  );
