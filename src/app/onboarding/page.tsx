...
export default function OnboardingPage() {
  ...
  async function handleSubmitCode() {
    ...
    if (result.ok) {
      ...
      // router.replace は useEffect (role 監視) に任せる
    } else {
      ...
      <div className="max-w-2xl w-full bg-white rounded-2xl shadow-lg p-8">
        {step === 'select' && (
          <>
            ...
            <p className="text-gray-600 mb-6 text-sm">
              ...
              <span dir="rtl">يرجى تحديد دورك</span>
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {(Object.keys(LABELS) as UserRole[]).map((r) => (
                <button
                  ...
                  className="text-left border-2 border-gray-200 rounded-xl p-5 hover:border-emerald-500 hover:bg-emerald-50 transition disabled:opacity-50"
                >
        ...
        {step === 'code' && selectedRole && (
          <>
            ...
            <p className="text-gray-600 mb-6 text-sm">
              学校 / 担当者から配布されたコードを入力してください
            </p>
            <input
              type="text"
              value={code}
              onChange={(e) => setCode(e.target.value.toUpperCase())}
              placeholder="TEAC-EJSCAIRO01-XXXXXX"
